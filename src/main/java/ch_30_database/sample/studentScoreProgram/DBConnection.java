...
public class DBConnection {
    ...
    private static final String url = "jdbc:mariadb://localhost:3306/sample_java";
    ...
    public static Connection getConnection(){
        try{
            ...
            //2. 데이터베이스 연결
            Connection connection = DriverManager.getConnection(url, user, password);//데이터베이스에 접속
            log.info("[Success]");
            return connection;
        } catch (ClassNotFoundException e) { //드라이버 로딩 실패시
            e.printStackTrace();
            log.error("[Fail] DB Loading Error :0");
        } catch (SQLException e) { //실패시
            e.printStackTrace();
            log.error("[Fail] DB Connection Fail :( ");
        }
        return null;
    }
